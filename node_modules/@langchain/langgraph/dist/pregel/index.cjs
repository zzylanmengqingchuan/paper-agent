...
var Pregel = class extends PartialRunnable {
	...
	*/
	async stream(input, options) {
		...
		return new require_stream.IterableReadableStreamWithAbortSignal(options?.encoding === "text/event-stream" ? require_stream.toEventStream(stream) : stream, abortController, disposeCombinedSignal);
	}
	streamEvents(input, options, streamOptions) {
		...
		return new require_stream.IterableReadableStreamWithAbortSignal(super.streamEvents(input, config, streamOptions), abortController, disposeCombinedSignal);
	}
	/**
