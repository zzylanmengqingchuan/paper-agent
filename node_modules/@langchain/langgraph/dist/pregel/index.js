...
var Pregel = class extends PartialRunnable {
	...
	*/
	async stream(input, options) {
		...
		const { signal: combinedSignal, dispose: disposeCombinedSignal } = combineAbortSignals(options?.signal, abortController.signal);
		const config = {
		...
		return new IterableReadableStreamWithAbortSignal(options?.encoding === "text/event-stream" ? toEventStream(stream) : stream, abortController, disposeCombinedSignal);
	}
	streamEvents(input, options, streamOptions) {
		...
		return new IterableReadableStreamWithAbortSignal(super.streamEvents(input, config, streamOptions), abortController, disposeCombinedSignal);
	}
	/**
